...
<?php include("includes/header.php"); ?>
	...
	if($delete)
	{
	  delete_record_by_id('univarr_pages',$delete);
	}
	newpageform(); 
	...
	if($pagelists)
	 {
		...
		echo "<tr><th width='5%'>S.No</th><th>Title</th><th width='15%'>Category</th><th width='5%'>Edit</th><th width='5%'>Delete</th></tr>";
		
		for($i=0;$i<count($pagelists);$i++)
		{
		   echo "<tr onmouseover='this.className=\"tblmouseover\"'  onmouseout='this.className=\"tblmouseout\"'><td>".($i+1)."</td><td>".$pagelists[$i]['title']."</td><td>".$pagelists[$i]['category']."</td><td><a href='manage_page.php?edit=".$pagelists[$i]['id']."'>Edit</a></td><td align='center'><a href='manage_page.php?delete=".$pagelists[$i]['id']."' onclick='return confirmSubmit()'>Delete</a></td></tr>";
